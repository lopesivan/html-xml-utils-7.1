 ...
 * Created: 11 Apr 2000
 * Version: $Id: hxindex.c,v 1.22 2016/03/04 15:05:37 bbos Exp $
 *
 **/
#include "config.h"
#include <assert.h>
...
static void print_full_term(const Indexterm term)
{
  enum {TEXT, TAG, DQUOTE, SQUOTE} state;
  string h;
  int i;

  for (state = TEXT, i = 0; i < term->nrkeys; i++) {
    if (i != 0) fputs(", ", stdout);
    for (h = term->terms[i]; *h; h++)
      switch (state) {
      case TEXT:
	if (*h == '<') state = TAG;
	else if (*h == '"') fputs("&quot;", stdout);
...
static void load_index(const string indexdb, Indexterm *terms)
{
  ...
  /* TODO: allow for arbitrarily long lines instead of MAXSTR */
...
int main(int argc, char *argv[])
{
  ...
  while ((c = getopt(argc, argv, "txb:i:cnfrs:u:O:X:")) != -1)
  switch (c) {
