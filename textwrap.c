 ...
 * Bert Bos
 * Created 10 May 1998
 * $Id: textwrap.c,v 1.29 2016/10/17 17:43:17 bbos Exp $
 */
#include "config.h"
#include <stdio.h>
#include <ctype.h>
...
EXPORT void flush()
{
  ...
  while (len != 0 && linelen + bufchars >= maxlinelen) { /* Need break */
    ...
    for (i = -1, j = 0, k = 0; j < len && (k < maxlinelen || i == -1); j++)
      ...
      else if ((buf[j] & 0x80) == 0 || (buf[j] & 0xC0) == 0xC0) k++; /* Char */
	...
	if ((buf[j] & 0x80) == 0 || (buf[j] & 0xC0) == 0xC0) bufchars--;
      }
    putchar('\n');				/* Break line */
    linelen = 0;
    assert(level >= 0);
  ...
  for (j = 0; j < len; j++)
    ...
    else if ((buf[j] & 0x80) == 0) {putchar(buf[j]); linelen++;}
    else if ((buf[j] & 0xC0) == 0xC0) {putchar(buf[j]); linelen++;}
    else putchar(buf[j]);
  bufchars = 0;
  len = 0;
}
...
EXPORT void outc(char c, bool preformatted)
{
  ...
  while (len >= buflen) {buflen += 1024; renewarray(buf, buflen);}
    if ((c & 0x80) == 0 || (c & 0xC0) == 0xC0) bufchars++; /* Character */
